...
class SdkTracerSuite extends CatsEffectSuite {
  ...
  sdkTest("set error status on abnormal termination (canceled)") { sdk =>
    for {
      tracer <- sdk.tracerProvider.get("tracer")
      ...
      assertEquals(spans.map(_.events.elements.isEmpty), List(true))
    }
  }
  ...
  sdkTest("set attributes only once") { sdk =>
    ...
    for {
      ...
      assertEquals(spans.flatMap(_.attributes.elements.toList), List(attribute))
    }
  }
  ...
  sdkTest("`currentSpanOrNoop` inside a span") { sdk =>
    ...
    TestControl.executeEmbed {
      for {
        ...
        assertEquals(spans.map(_.attributes.elements), List(Attributes(attribute)))
      }
    }
  }
      ...
      SpanNode(
        ...
        List(
          SpanNode(
            ...
            start,
            acquireEnd,
            List(SpanNode("acquire-inner", start, acquireEnd, Nil))
          ),
          SpanNode(
            "use",
            body1Start,
          ...
          SpanNode("release", body3End, end, Nil)
        )
      )
    }
  ...
  sdkTest("trace resource with surround") { sdk =>
    ...
    def expected(start: FiniteDuration) = {
      ...
      SpanNode(
        ...
        List(
          SpanNode("acquire", start, acquireEnd, Nil),
          SpanNode(
            "use",
            bodyStart,
            bodyEnd,
            List(SpanNode("body", bodyStart, bodyEnd, Nil))
          ),
          SpanNode("release", bodyEnd, end, Nil)
        )
      )
    }
    ...
    TestControl.executeEmbed {
      for {
        now <- IO.monotonic.delayBy(1.second) // otherwise returns 0
        tracer <- sdk.tracerProvider.tracer("tracer").get
  ...
  sdkTest("trace resource with use_") { sdk =>
    ...
    def expected(start: FiniteDuration) = {
      ...
      SpanNode(
        ...
        List(
          SpanNode("acquire", start, acquireEnd, Nil),
          SpanNode("use", acquireEnd, acquireEnd, Nil),
          SpanNode("release", acquireEnd, end, Nil)
        )
      )
    }
  ...
  sdkTest(
    ...
    for {
      ...
      spans.flatMap(_.attributes.elements).toSet,
  ...
  sdkTest("nested SpanOps#surround for liftTo[IO]") { sdk =>
    TestControl.executeEmbed {
      for {
        now <- IO.monotonic.delayBy(1.second) // otherwise returns 0
        tracerIO <- sdk.tracerProvider.get("tracer")
  ...
  protected def sdkTest[A](
      options: TestOptions,
      configure: SdkTracerProvider.Builder[IO] => SdkTracerProvider.Builder[IO] = identity,
      additionalPropagators: List[TextMapPropagator[Context]] = Nil
  )(body: TracesTestkit[IO] => IO[A])(implicit loc: Location): Unit =
    test(options)(makeTestkit(configure, additionalPropagators).use(body))

  protected def makeTestkit(
      configure: SdkTracerProvider.Builder[IO] => SdkTracerProvider.Builder[IO],
      additionalPropagators: List[TextMapPropagator[Context]]
  ): Resource[IO, TracesTestkit[IO]] =
    TracesTestkit
      .builder[IO]
  ...
  private object NestedSurround {
    ...
    def expected(start: FiniteDuration): SpanNode = {
      ...
      SpanNode(
        ...
        List(
          SpanNode("body-1", body1Start, body1End, Nil),
          SpanNode("body-2", body1End, end, Nil)
        )
      )
    }
  }
