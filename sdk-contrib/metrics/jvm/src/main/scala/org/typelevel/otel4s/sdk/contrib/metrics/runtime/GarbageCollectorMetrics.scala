...
private object GarbageCollectorMetrics {
  ...
  def register[F[_]: Async: Meter: Diagnostic](
    ...
    Async[F]
      ...
      .toResource
      .ifM(create[F](bucketBoundaries), warn[F].toResource)

  private def create[F[_]: Async: Meter](
      bucketBoundaries: BucketBoundaries
  ): Resource[F, Unit] =
    for {
      beans <- Async[F].delay(ManagementFactory.getGarbageCollectorMXBeans).toResource
      dispatcher <- Dispatcher.sequential[F]
      histogram <- JvmMetrics.GcDuration.create[F, Double](bucketBoundaries).toResource
  ...
  private def warn[F[_]: Diagnostic]: F[Unit] =
    Diagnostic[F].error(
